...
test('六段式导入备用池并加密保存全部凭据，列表标记密码与 2FA', async () => {
  await start();
  const res = await importCall({ text: `${LINE_A}----GPT#Pass!----jbswy3dpehpk3pxp` });
  assert.equal(res.statusCode, 201);
  assert.equal(res.json().created, 1);
  assert.deepEqual(res.json().invalid_lines, []);
  const account = accountRow('a@test.com');
  assert.equal(account.pool, 'reserve');
  assert.equal(account.tokens_enc, null);
  assert.match(account.credentials_enc, /^v1:/);
  assert.deepEqual(ctx.crypto.decryptJson(account.credentials_enc, 'accounts.credentials_enc'), {
    outlook: {
      password: 'pw1',
    ...
    totp_pickup_code: 'JBSWY3DPEHPK3PXP',
  });
  const list = await ctx.app.inject({ method: 'GET', url: '/api/v1/accounts?pool=reserve' });
  assert.equal(list.statusCode, 200);
  assert.equal(list.json().items[0].has_password, true);
  assert.equal(list.json().items[0].has_2fa, true);
  assert.equal(list.body.includes('GPT#Pass!'), false);
  assert.equal(res.body.includes('JBSWY3DPEHPK3PXP'), false);
});

test('备用池重复导入六段式补全凭据，后续四段式或空尾段不清空密码和 2FA', async () => {
  await start();
  await importCall({ text: LINE_A });
  const res = await importCall({ text: `${LINE_A}----gpt-pass----JBSWY3DPEHPK3PXP` });
  assert.equal(res.json().created, 0);
  assert.deepEqual(res.json().duplicates_in_reserve, ['a@test.com']);
  const expected = ctx.crypto.decryptJson(accountRow('a@test.com').credentials_enc, 'accounts.credentials_enc');
  assert.equal(expected.password, 'gpt-pass');
  assert.equal(expected.totp_secret, 'JBSWY3DPEHPK3PXP');
  assert.equal(expected.outlook.refresh_token, 'r'.repeat(120));
  for (const text of [LINE_A, `${LINE_A}--------`]) {
    await importCall({ text });
    assert.deepEqual(ctx.crypto.decryptJson(accountRow('a@test.com').credentials_enc, 'accounts.credentials_enc'), expected);
  }
});

test('主号池重复导入六段式同时更新密码、2FA 密钥和取件码', async () => {
  await start([remoteAccount(101, 'a@test.com')]);
  await importCall({ text: LINE_A, adopt_remote: true });
  const res = await importCall({ text: `${LINE_A}----gpt-pass----JBSWY3DPEHPK3PXP` });
  assert.equal(res.json().created, 0);
  assert.deepEqual(res.json().duplicates_in_main, ['a@test.com']);
  const account = accountRow('a@test.com');
  assert.equal(account.pool, 'main');
  const credentials = ctx.crypto.decryptJson(account.credentials_enc, 'accounts.credentials_enc');
  assert.equal(credentials.password, 'gpt-pass');
  assert.equal(credentials.totp_secret, 'JBSWY3DPEHPK3PXP');
  assert.equal(credentials.totp_pickup_code, 'JBSWY3DPEHPK3PXP');
  assert.equal(credentials.outlook.refresh_token, 'r'.repeat(120));
});

test('非法六段式不入库，同批合法账号继续导入', async () => {
  await start();
  const res = await importCall({ text: `${LINE_A}----gpt-pass----invalid-2fa\n${LINE_B}` });
  assert.equal(res.statusCode, 201);
  assert.equal(res.json().created, 1);
  assert.equal(res.json().invalid_lines[0].line, 1);
  assert.match(res.json().invalid_lines[0].reason, /2FA.*Base32/);
  assert.equal(accountRow('a@test.com'), undefined);
