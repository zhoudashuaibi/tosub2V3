...
function TableRow({ className, ...props }: React.ComponentProps<'tr'>) {
  return (
    <tr
      data-slot="table-row"
      className={cn('border-b transition-colors hover:bg-muted/60 data-[state=selected]:bg-accent/60', className)}
      {...props}
    />
  );
}
...
function TableHead({ className, ...props }: React.ComponentProps<'th'>) {
  return (
    <th
      ...
      className={cn(
        'h-10 whitespace-nowrap px-3 text-left align-middle text-[11px] font-medium tracking-[0.04em] text-muted-foreground',
        className,
      )}
      {...props}
    />
