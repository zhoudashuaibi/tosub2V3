...
`email / discard_reason(徽章) / discard_detail(悬浮) / balance(废弃时余额) / discarded_at / proxy_name·proxy_user(废弃时的出口代理) / 操作(移回主池·彻底删除)`。
...
- **取值优先级**（`modules/accounts/discard-usage.js` 的 `snapshotDiscardProxy`）：
...
- 远端只给 `proxy_id` 时，名字与认证账号由 `remoteSync.resolveDiscardProxy` 查代理列表补齐（60s 缓存，批量废弃不放大请求量）。**认证账号（proxy.username）与代理名一起看**：同一台机器换个认证账号就是另一条出口。
