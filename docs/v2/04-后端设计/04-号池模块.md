...
统一 JSON 导入（sub2api 账号导出文件，`{ accounts: [...] }` 或裸数组）：

```jsonc
{
  "accounts": [{
    "name": "a@b.com----取件密码----GPT密码",       // 兜底：第 1 段邮箱、第 3 段 GPT 密码
    "notes": "{ \"mailbox\": { \"bind_email\": \"a@b.com\", \"password\": \"邮箱密码\",
                \"client_id\": \"<uuid>\", \"refresh_token\": \"...\" },
              \"gpt\": { \"password\": \"GPT登录密码\" },
              \"two_factor\": { \"enabled\": true, \"secret\": \"<Base32>\" } }",  // JSON 字符串
    "credentials": { "refresh_token": "...", "access_token": "..." }  // OAuth tokens（忽略）
  }]
}
```

字段映射（notes 为准，name/credentials 兜底）：

| 来源 | 落库字段 | 说明 |
|---|---|---|
| `notes.mailbox.bind_email` | `email` | 邮箱（四段第 1 段） |
| `notes.mailbox.password` | `credentials.outlook.password` | **邮箱密码**（四段第 2 段），勿与 GPT 密码混淆 |
| `notes.mailbox.client_id` | `credentials.outlook.client_id` | 四段第 3 段，须为 UUID |
| `notes.mailbox.refresh_token` | `credentials.outlook.refresh_token` | 四段第 4 段，长度 ≥100 |
| `notes.gpt.password` | `credentials.password` | ChatGPT 登录密码 |
| `notes.two_factor.enabled + secret` | `credentials.totp_secret` + `totp_pickup_code` | 开启两步验证时，密钥同时作本地 TOTP 与在线取件码（Base32 校验） |
| `credentials.access_token / refresh_token` | —（**忽略**） | 原登录态不带入；加入主号池必须走本系统 join-main 登录授权 |

条目全部进备用号池（`pool='reserve'`）。仍兼容旧输入：tosubV2 跨实例导出（`type: tosub2-accounts`，带 tokens 的条目依旧直入主号池）与 v1 四段行格式（`邮箱----邮箱密码----clientId----refresh_token`，逐行）；`twofa_text` / `passwords_text` 增量补录参数保留（API 兼容，前端已并入 JSON 导入）。
...
|---|---|---|
| sub2api 账号错误含 deactivated/banned/suspended（分类正则见 sub2api 配置）**且邮箱辅证（封禁邮件）证实**；未证实只暂停远端调度保留观察 | `banned_401` | 监控巡检 |
| sub2api 账号错误含 429/rate limit/too many requests | `rate_limited_429` | 监控巡检 |
| 加入主池登录时返回永久封禁 | `login_failed` | 任务引擎回调 |
| 手动废弃 | `manual` | 前端按钮 |

> **自动修复连败不再废弃**（v2.1 起）：连续失败 `max_repair_attempts` 次改为**暂停保留待重授**——`status='needs_reauth'` + `auto_repair_blocked=1`（`account_events(type='repair_parked')`）+ 暂停远端调度；重授成功（`join_succeeded` / `pushRepairedCredentials`）自动清封锁恢复调度。`repair_failed` 仍为合法 discard_reason（历史数据/手动场景）。

> **401 ≠ 封禁**：401 只代表会话过期，巡检一律走自动修复——有 refresh_token 先刷新（401 失败由引擎自动转完整登录），没有直接发完整登录；`banned_patterns` 中的裸 `401` 会在巡检时被剔除。
...
`email / discard_reason(徽章) / discard_detail(悬浮) / balance(废弃时余额) / discarded_at / proxy_name·proxy_user(废弃时的出口代理) / codex_fingerprint_mode(封号时的 Codex 指纹收敛档位) / 操作(移回主池·彻底删除)`。

### 5.4 废弃时的出口代理快照（IP 归因）

废弃池列表的「代理 IP」列，回答的是「这个号死在哪个出口上」（同一个 IP 上死了一批号 = 该 IP 被拉黑的信号）。

- **只在废弃那一刻取值**：新增 `accounts.discard_proxy_name / discard_proxy_user / discard_proxy_id / discard_proxy_at`（迁移 `0011_discard_proxy.sql`），废弃时抓一次后不再改写。远端绑定会被「一键更换代理 IP」改绑并删掉旧代理，号废弃后也可能被清理，事后现查只会得到「现在绑的是哪条」。
- **取值优先级**（`modules/accounts/discard-usage.js` 的 `snapshotDiscardFacts`）：
  1. 调用方在废弃当下传进来的远端账号对象 —— `pools.moveToDiscard(id, reason, detail, { proxy })` / `joinFailed(..., { proxy })`，巡检本来就有这个对象；
  2. 调用方没给（手动批量废弃、登录终局失败）→ 按 `sub2api_account_id` 单查远端账号（1 次请求，不拉全量列表），否则按 email 走远端索引；
  3. 都没有 → 退回本机 tosub2 代理：该号最后一条用过代理的任务 `jobs.proxy_id` → `proxies.label` + `display_url` 里的认证账号（登录类废弃的号多数没上过远端，这是唯一能拿到的出口线索）。
- 远端只给 `proxy_id` 时，名字与认证账号由 `remoteSync.resolveDiscardRemote` 查代理列表补齐（60s 缓存，批量废弃不放大请求量）。**认证账号（proxy.username）与代理名一起看**：同一台机器换个认证账号就是另一条出口。
- 拿不到就留空，不反推成「直连」；代理快照整段 best-effort 吞异常，失败只留 debug 日志，绝不影响废弃流转与用量快照。没配 sub2api 时钩子照跑（本机任务那半边仍有效）。
- 回主池时快照随 `discard_reason/discarded_at` 一起清空（`restore`）——下次废弃会重新抓。
- 审计：`account_events(type='discard_proxy_snapshot', {proxy_id, name, username})`，与 `moved_to_discard` 分开记。列表搜索框 `q` 在废弃池同时匹配 email / 代理名 / 认证账号。

### 5.5 封号时的 Codex 指纹收敛快照（档位归因）

废弃池列表的「Codex 指纹收敛」列，回答的是「这个号被封的时候，远端给它开的是哪一档收敛」
（同一档收敛下死了一批号 = 该档位可疑的信号）。

- **只在废弃那一刻取值**：新增 `accounts.discard_codex_fingerprint_mode / discard_codex_fingerprint_at`（迁移 `0012_discard_codex_fingerprint.sql`）。档位只存在远端账号 `extra.codex_fingerprint_mode` 里，本地库没有第二份；而它在 sub2api 账号编辑页随时能改、号废弃后远端记录也可能被清理，事后现查只能得到「现在是什么」。
- **取值优先级**（`modules/accounts/discard-usage.js` 的 `snapshotDiscardFacts`，与出口代理**共用同一次远端解析** —— 两者在同一个远端账号对象上，分两次查等于把同一个号查两遍）：
  1. 调用方在废弃当下传进来的远端账号对象 `extra.codex_fingerprint_mode`（巡检 401/429 路径本来就有这个对象 → **零额外请求**；对象里连 `extra` 都没有时才回落 2）；
  2. `remoteSync.resolveDiscardRemote` 单账号接口 → 有上限的邮箱查找（与代理同一套回退，见 §5.4）。
- **键缺失 ≠ 读不到**（本列最容易搞错的一点）：sub2api 契约里 `off`（透传）就是不写这个键（上传侧 `buildPayload` 同样如此），所以「拿到了 `extra` 但没有这个键」是**确定**的 `off`，照实显示「关闭（透传）」；只有连 `extra` 都拿不到（从未上传过远端 / 远端账号已删除 / 远端对象不带该字段）才留空，UI 显示 `—`，**不代填成 off**。
- 非法值按 sub2api 读取侧与上传侧同一口径归为 `off`（`normalizeCodexFingerprintMode`）。
- 回主池时随 `discard_reason/discarded_at` 一起清空（`restore`）；拿不到就不写事件，避免「事件说有档位、列却是空的」。
- 审计：`account_events(type='discard_codex_fingerprint_snapshot', {mode})`，与 `moved_to_discard` / `discard_proxy_snapshot` 分开记。
- 排序白名单键 `codex_fingerprint_mode`：**按收敛强度排**而不是字典序（字典序是 `device < full < off < session`，`off` 夹在中间读不出「越来越收敛」），映射 `off/device/session/full → 0/1/2/3`；两段式与 `used_amount` / `proxy_name` 同款 —— 方向只作用于最后一个表达式，所以「是否为空」分组键固定 ASC，读不到档位的号两个方向都沉底。
