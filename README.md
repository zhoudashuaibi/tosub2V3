...
| 备用号池 | Outlook 四段 / 六段导入（六段附带 ChatGPT 密码与 2FA 密钥，三重查重）/ sub2api·redeem 导出导入（带 tokens 直入主池，纯邮箱进备用池）、邮件初始化（初始余额 credits/25 + 封禁关键字）、单/批量加入主池（远程登录授权） |
...
文本导入每行一个账号，可混用四段式和六段式，支持空行与 `#` 注释行：

```text
邮箱----邮箱密码----clientId----refreshToken
邮箱----邮箱密码----clientId----refreshToken----ChatGPT密码----2FA密钥
```

六段式的第 2 段是邮箱密码，第 5 段是 ChatGPT 登录密码，第 6 段是 Base32 格式的 2FA 密钥（16–128 位，支持小写、空格与末尾填充符 `=`，保存时统一规范化）。第 5、6 段可留空，但需保留分隔符；重复导入备用号会更新已提供的凭据，空值不会清除已有的 ChatGPT 密码或 2FA。字段本身含 `----` 时请使用 JSON 导入，避免分段歧义。

仍支持 sub2api / redeem 和 tosubV2 账号导出 JSON；JSON 中带 OAuth tokens 的账号直接进入主号池。
